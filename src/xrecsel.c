...
typedef struct
{
   ...
   uint32_t nbOccur;                 /* nb. occurences de la cle dans une colonne du tableau  */
   uint32_t *ind;                    /* tableau d'indices montrant les occur. de la cle dans une colonne
                                   du tableau des records. */
   } cleInfoStruct;

typedef struct
{
   uint32_t nbCles;                  /* nb de cles distinctes dans une colonne du tableau */
   cleInfoStruct *cleInfo;      /* infos associees a cette cle */
   } cleStruct;

typedef struct
{
   uint32_t indDes;                  /* indice du descripteur */
   uint32_t indCle;                  /* indice de la cle      */
   }  filtresStruct;

typedef struct
{
   uint32_t indDes;                  /* indice du descripteur */
   uint32_t nbFiltresListe;          /* nb de cles utilisees pour la filtration de records, pour un 
                                   descripteur donne */
   uint32_t *liste;                  /* liste des indices de cles utilises pour la filtration de records */ 
   }  filtresInfoStruct;
...
typedef struct
{
   ...
   uint32_t           nbDes;                         /* nb total de descripteurs (de colonnes) dans le tableau */
   uint32_t           lastNbRecs;
   uint32_t           nbRecs;                        /* nb total de records dans le tableau */
   uint32_t           nbRecsFiltres;                 /* nb total de records repondant aux criteres de selection */
   uint32_t           nbCles;                        /* nb total de cles (dans toutes les colonnes du tableau) */
   uint32_t           nbFiltres;                     /* nb total de filtres en vigueur */
   uint32_t           nbRecsSelect;                  /* nb total de recordss selectionnes par l'usager. Cette
                                                  operation se fait par un click de la souris. */
   uint32_t           *indRecsFiltres;               /* tableau contenant les indices des records repondant aux
                                                   criteres de selection */
   uint32_t           *indRecsSelect;                /* tableau contenant les indices des records selectionnes par 
                                                   l'usager */
   uint32_t           typeSelection;
   uint32_t           statutBoutonFermer;
   uint32_t           StatutSelection;               /* = SELECTION_TERMINEE quand on choisis Apply */
...
uint32_t largeurMenus[NB_MAX_DESCRIPTEURS];
static uint32_t               wi;                    /* L'indice de la fenetre dans laquelle on travaille.   */
...
uint32_t f77name(xselact)(uint32_t sel[], uint32_t *nbsel, uint32_t *indsel);
uint32_t f77name(xselfer)(uint32_t sel[], uint32_t *nbsel, char idents[], uint32_t table[][3], uint32_t *m, uint32_t *n, uint32_t     *indsel, F2Cl len);
uint32_t f77name(xselins)(char *tableau, uint32_t table[][3], uint32_t *nbrecs, F2Cl lentableau); 
uint32_t f77name(xselopt)(uint32_t *indSelecteur, char option[], char valeur[], uint32_t lenOption, F2Cl lenValeur);
uint32_t f77name(xselouf)(uint32_t table[][3], uint32_t *nbrecs);
uint32_t f77name(xseloup)(char *titre, uint32_t  *nbrecs, char idents[], uint32_t *nbdes, uint32_t *indSel, uint32_t *typeSel, F2Cl lenNomFich, F2Cl lenIdents);
uint32_t f77name(xselupd)();
uint32_t f77name(xseldim)();
uint32_t f77name(xselundim)();
uint32_t f77name(xselup)(uint32_t *indSelecteur);
uint32_t f77name(xseldown)(uint32_t *indSelecteur);
uint32_t c_xselup(uint32_t indSelecteur);
uint32_t c_xselopt(uint32_t indSelecteur, char *option, char *valeur);
uint32_t c_xseldown(uint32_t indSelecteur);
...
uint32_t     ActiverSelWidgets();
void        AfficherListe(Widget w, XmStringTable items, uint32_t nbItems);
void        AfficherNbSelect(uint32_t nb);
uint32_t     AjouterCle(cleStruct cles[], uint32_t pos, uint32_t indDes, uint32_t indCle);
uint32_t     AjouterFiltre(uint32_t indDes, uint32_t indCle);
uint32_t     AjouterNouvelleCle(char *val, uint32_t len, cleStruct cles[], uint32_t indDes, uint32_t indCle);
static void AjusterSensibiliteBoutons();
            CalculerLargeurMenus(uint32_t largeurMenus[], uint32_t table[][3]);
uint32_t     ChangerWidgets();
uint32_t     ChercherCle(char *val, uint32_t len, cleStruct cles[], uint32_t indDes);
uint32_t     ComparerCles(cleInfoStruct *cleInfo1, cleInfoStruct *cleInfo2);
uint32_t     ComparerFiltres(filtresStruct *filtre1,  filtresStruct *filtre2);
uint32_t     DesactiverSelWidgets();
uint32_t     EnleverFiltre(uint32_t indDes, uint32_t indCle);
uint32_t     FiltrerRecords(uint32_t sel[], uint32_t *nbsel, XmString **recs, cleStruct cles[], filtresStruct *filtres, uint32_t nbRecs, uint32_t nbDes, uint32_t  nbFiltres);
uint32_t     FreeCles(cleStruct cles[], uint32_t n);
uint32_t     Freerecs(XmString *(**recs), uint32_t nbDes);
void        FreerecsAffiches(XmString **recsAffiches, uint32_t nbDes);
uint32_t     InitCles(char *tableau, uint32_t  table[][3], cleStruct cles[], uint32_t nbRecs, uint32_t nbDes);
uint32_t     InitFiltresInfo(filtresInfoStruct filtresInfo[], filtresStruct *filtres, uint32_t nbFiltres);
uint32_t     InitRecs(XmString *(**recs), cleStruct cles[], uint32_t nbRecs, uint32_t nbDes);
void        InitRecsAffiches(XmString **recsAffiches, char *tableau, uint32_t table[][3], uint32_t nbRecs);
void        InitRecsFiltresAffiches(XmString **recsFiltresAffiches, XmString *recsAffiches, uint32_t nbRecs);
uint32_t     InitTitresMenus(char *idents[], uint32_t nbDes);
uint32_t     InitWidgetsAll1(XmString TitresMenus[], uint32_t nbRecsFiltres, uint32_t nbDes);
uint32_t     InitWidgetsAll2(XmString TitresMenus[], uint32_t nbRecsFiltres, uint32_t nbDes);
uint32_t     InitWidgetsBasic();
void        InitWidgetsBouton();
uint32_t     InitWidgetsCallback(uint32_t nbDes);
uint32_t     InitWidgetsFiltre();
void        InitWidgetsForm();
void        InitWidgetsMenu(XmString TitresMenus[], uint32_t nbDes);
uint32_t     InitWidgetsRec(uint32_t nbRecsFiltres,  uint32_t nbDes);
uint32_t     LibererPointeurs();
uint32_t     LibererPanneauListe();
uint32_t     MessageChargement(uint32_t nbRecs1, uint32_t nbRecs2);
uint32_t     NettoyerString(char str[]);
static void PositionnerMenubar();
static void TrouverLongueurMot(uint32_t *longueur, XmString mot);
Widget      TrouverWidgetParent();
uint32_t     UpdateFiltres();
void        UpdateRecsFiltresAffiches(XmString **recsFiltresAffiches, XmString *recsAffiches, uint32_t nbDes, uint32_t sel[], uint32_t nbSel);
uint32_t     XSelectstdActiver();
uint32_t     SelectstdFermer();
Widget      XSelectstdOuvrir();

uint32_t XSelectstdActiver(uint32_t  sel[], uint32_t  *nbsel, uint32_t  *indSelecteur);
uint32_t XSelectstdCommencerInit(char *titre, uint32_t nbrecs, char **idents, uint32_t nbdes, uint32_t indSel, uint32_t typeSel);
uint32_t XSelectstdFermer(uint32_t  sel[], uint32_t  *nbsel, char **idents, uint32_t  table[][3], uint32_t  *m, uint32_t  *n, uint32_t  *indSelecteur);
void XSelectstdInserer(char *tableau, uint32_t table[][3], uint32_t nbrecs);
void XSelectstdTerminerInit(uint32_t table[][3], uint32_t nbrecs);
/*===============================================================================
...
static void AfficherInfoFiltres(Widget w, void* unused1, void* unused2)
{
   ...
   uint32_t          i;              /* Nombre de resources utilisees.                       */
...
{
   uint32_t  j;       /* Compteur.                                   */
   uint32_t  nb;      /* Nombre de records affiches.                 */
   uint32_t  i;       /* Nombre de resources utilisees; compteur.    */
   Arg  args[5]; /* Resources des widgets qui sont utilisees.   */
   uint32_t zero = 0;
...
static XtCallbackProc HighlightFields(Widget  w, void* unused1, void* call_data)
{
   uint32_t                  i, j;                                      /* Compteurs.                                */
   uint32_t                  numItem;                                   /* La position de l'item (de)selectionne.    */
   uint32_t                  *itemPos;
...
void AfficherListe(Widget w, XmStringTable items, uint32_t nbItems)
{
   uint32_t nb, cnt; /* Nombre d'items visibles.                     */
   uint32_t i;       /* Nombre de resources utilisees.               */
   Arg args[5]; /* Resources des widgets qui sont utilisees.    */
...
uint32_t AjouterCle(cleStruct cles[], uint32_t pos, uint32_t indDes, uint32_t indCle)
{
   uint32_t nbElem; /* Nombre d'elements dans la liste des indices ou la cle se trouve dans tableau[]. */

   nbElem                                       = ++cles[indDes].cleInfo[pos].nbOccur;
   cles[indDes].cleInfo[pos].ind                = (uint32_t *)realloc(cles[indDes].cleInfo[pos].ind, nbElem*sizeof(int));
...
uint32_t AjouterNouvelleCle(char *val, uint32_t len, cleStruct cles[], uint32_t indDes, uint32_t indCle)
{
   uint32_t ClesInd; /* Le nombres de cles dans le tableau de descripteur. */
   ...
   cles[indDes].cleInfo[ClesInd].ind            = (uint32_t *)calloc(1, sizeof(int));
...
uint32_t ChercherCle(char *val, uint32_t len, cleStruct cles[], uint32_t indDes)
{
   uint32_t i;       /* Position dans le tableau du descripteur.        */
   uint32_t nbCles;  /* Nombres de cles dans le tableau du descripteur. */
...
{
   uint32_t i;         /* Compteur.                                          */
   uint32_t indFiltre; /* La position du filtre dans la liste des filtres.   */
...
uint32_t FiltrerRecords(uint32_t sel[], uint32_t *nbsel, XmString **recs, cleStruct cles[], filtresStruct *filtres, uint32_t nbRecs, uint32_t nbDes, uint32_t  nbFiltres)
{
   uint32_t                  i, j, k;                          /* Compteurs. */
   uint32_t                  clePasTrouvee;                    /* Vrai quand on trouve la cle. */
   uint32_t                  nbFiltresDistincts;               /*  */
...
{
   uint32_t i, j;            /* Compteurs.                                                   */
   uint32_t indDesCourant;   /* L'indice du descripteur qui est courament en consideration.  */
   uint32_t indListe;        /*  */
   ...
   indDesCourant = 0;
   filtresInfo[indDesCourant].liste = (uint32_t *)calloc(xs[wi].cles[filtres[0].indDes].nbCles, sizeof(int));
   ...
   for (i = 1; i < nbFiltres; i++)
      ...
      if (filtresInfo[indDesCourant].indDes != filtres[i].indDes)
         {
         indDesCourant++;
         filtresInfo[indDesCourant].liste = (uint32_t *)calloc(xs[wi].cles[filtres[i].indDes].nbCles, sizeof(int));
...
void InitRecsAffiches(XmString **recsAffiches, char *tableau, uint32_t table[][3], uint32_t nbRecs)
{
   ...
   uint32_t  offset, longueur, ioffset;
...
{
   uint32_t i, j;  /* Compteurs.                                             */
   uint32_t index; /* Utilise pour encoder les indices de menuItems[][].     */
...
void InitWidgetsListe()
{
   ...
   uint32_t i;       /* Nombre de resources utilisees.               */
   uint32_t j, k;    /* Compteurs.                                   */
   uint32_t h;       /* La hauteur de menuform.                      */
   char *text;
   XmString label;
...
uint32_t InitWidgetsRec(uint32_t nbRecsFiltres,  uint32_t nbDes)
{
   ...
   uint32_t  i;              /* Nombre de resources utilisees.               */
   uint32_t  j;              /* Compteurs.                                   */
   char nomListe[16];  /* Le nom d'un widget dans liste.               */
   uint32_t  nb, cnt;        /* Le nombre de records visibles.               */
   uint32_t lng;
...
MessageChargement(uint32_t nbRecs1, uint32_t nbRecs2)
{
   Arg args[10];
   uint32_t i;
   Position x, y;
...
uint32_t UpdateFiltres()
{
   uint32_t i,j;   /* Compteur.                        */
   uint32_t res; /* Le nombre de records filtres.    */
   uint32_t nb;  /* Le nombre de records visibles.   */
   
   uint32_t numItems, *selectedItems, nbTotalItems;
...
f77name(xseloup)(char *titre, uint32_t  *nbrecs, char idents[], uint32_t *nbdes, uint32_t *indSel, uint32_t *typeSel, F2Cl lenNomFich, F2Cl lenIdents)
{
...
f77name(xselins)(char *tableau, uint32_t table[][3], uint32_t *nbrecs, F2Cl lentableau)
{
   XSelectstdInserer(tableau, table, *nbrecs);
   }

f77name(xselouf)(uint32_t table[][3], uint32_t *nbrecs)
{
...
/*======================================================================================================*/
XSelectstdCommencerInit(char *titre, uint32_t nbrecs, char **idents, uint32_t nbdes, uint32_t indSel, uint32_t typeSel)
{
   uint32_t i;
   ...
   if (nbrecs > 0)
      {
      xs[wi].indRecsFiltres        = (uint32_t *)calloc(xs[wi].nbRecs, sizeof(int));
      xs[wi].indRecsSelect         = (uint32_t *)calloc(xs[wi].nbRecs, sizeof(int));
...
f77name(xselup)(uint32_t *indSelecteur)
{
...
c_xselup(uint32_t indSelecteur)
{
   XtRealizeWidget(xs[indSelecteur].topLevel);
   }
...
f77name(xseldown)(uint32_t *indSelecteur)
{
...
c_xseldown(uint32_t indSelecteur)
{
   XtRealizeWidget(xs[indSelecteur].topLevel);
   }
...
f77name(xselopt)(uint32_t *indSelecteur, char option[], char valeur[], F2Cl flenOption, F2Cl flenValeur)
{
   uint32_t lenOption=flenOption, lenValeur=flenValeur;
...
c_xselopt(uint32_t indSelecteur, char *option, char *valeur)
{
